...
const changeSize = function (event, button, newStrapLength) {
  console.log(newStrapLength);
  ...
  let status = button.parentElement.querySelector(".backpack__strap span");
  status.innerText = newStrapLength;
};
...
const newStrapLength = (strapArray) => {
  ...
  strapArray.forEach((listElement) => {
    ...
    lengthForm.innerHTML = `
      ...
      <button class="feature update-size">Update</button>
    `;
...
const backpackList = backpackObjectArray.map((backpack) => {
  ...
  let updateButton = backpackArticle.querySelector(".update-size");
  ...
  updateButton.addEventListener("data-side", (event) => {
    changeSize(event, updateButton, newStrapLength);
  });
